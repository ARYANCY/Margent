...
export const AgentNode = memo(({ data }: AgentNodeProps) => {
    ...
    switch (agent.type) {
      case "qml":
        return {
          ...
          activeColor: "border-pink-500 ring-2 ring-pink-100"
        };
      case "ml":
        return {
          ...
          activeColor: "border-sky-500 ring-2 ring-sky-100"
        };
      case "groq":
        return {
      ...
      default:
        return {
          ...
          activeColor: "border-slate-800 ring-2 ring-slate-100"
        };
    }
  };
  ...
  return (
    <div
      className={`relative w-64 h-[142px] px-3 py-2.5 bg-white flex flex-col justify-between select-none transition-all duration-150 ${
        isSelected
          ? "border-2 border-slate-900 shadow-md ring-2 ring-slate-200 scale-102 z-30"
          : isActive
          ? `border-2 border-slate-800 shadow-md ring-2 ${meta.activeColor} scale-101 z-20`
          : "border border-slate-200 hover:border-slate-400 shadow-sm"
      }`}
    >
      {/* Handles */}
      <Handle type="source" position={Position.Top} id="source-top" className="!w-2 !h-2 !bg-slate-800 !border !border-white !rounded-none !-top-1" />
      <Handle type="target" position={Position.Top} id="target-top" className="!w-2 !h-2 !bg-slate-800 !border !border-white !rounded-none !-top-1" />
      <Handle type="source" position={Position.Bottom} id="source-bottom" className="!w-2 !h-2 !bg-slate-800 !border !border-white !rounded-none !-bottom-1" />
      <Handle type="target" position={Position.Bottom} id="target-bottom" className="!w-2 !h-2 !bg-slate-800 !border !border-white !rounded-none !-bottom-1" />

      {/* Left Colored Stripe */}
      <div className={`absolute top-0 left-0 bottom-0 w-1 ${meta.accentBar}`} />

      {/* Active Pulse Tag */}
      {isActive && (
        <div className="absolute -top-2 right-2 bg-slate-900 text-white px-1.5 py-0.2 border border-slate-700 text-[8px] font-mono font-bold uppercase flex items-center gap-1 shadow-sm">
      ...
      <div className="flex items-center justify-between border-b border-slate-100 pb-1.5 pl-1.5">
        <div className={`flex items-center space-x-1.5 px-1.5 py-0.2 border text-[9px] font-mono uppercase ${meta.badge}`}>
          {meta.icon}
          <span>{meta.label}</span>
        </div>
        <span className="text-[10px] font-mono font-bold text-slate-500">#{agent.agentId}</span>
      </div>
