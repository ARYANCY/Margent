...
export const AdminNode = memo(({ data }: AdminNodeProps) => {
  ...
  return (
    <div className="relative px-5 py-4 bg-white border-2 border-slate-800 shadow-lg w-[420px] select-none">
      {/* Handles */}
      ...
      {/* Header Badge */}
      {isActive && (
        <div className="absolute -top-2.5 left-1/2 -translate-x-1/2 bg-slate-900 text-white px-2.5 py-0.2 border border-slate-700 text-[8px] font-mono font-bold uppercase flex items-center gap-1.5 tracking-wider shadow-sm">
      ...
      {/* Consensus Decision */}
      <div className="p-2.5 bg-slate-50 border border-slate-200 mb-2.5">
        <div className="flex items-center justify-between mb-1 text-[10px] font-mono">
          ...
          <span className={`px-2 py-0.2 uppercase tracking-wider text-[10px] border ${decisionBadgeColor}`}>
            {adminAnalysis?.decision || "SCALE"}
          </span>
        </div>
