...
export const SimulationControls: React.FC = () => {
  ...
  return (
    <div className="flex items-center space-x-2.5 bg-white/95 backdrop-blur border border-slate-300 shadow-md px-3.5 py-2 select-none transition-all duration-200">
      {/* 1. Play / Pause Button */}
      <button
        onClick={isRunning ? pauseSimulation : startSimulation}
        className={`px-3.5 py-1.5 text-xs font-mono font-bold flex items-center gap-1.5 transition-all duration-150 uppercase tracking-wider shadow-sm border ${
          isRunning
      ...
      <button
        ...
        disabled={isRunning}
        className="px-3 py-1.5 text-xs font-mono font-bold bg-white hover:bg-slate-50 text-slate-800 border border-slate-300 disabled:opacity-40 flex items-center gap-1 transition-all duration-150 uppercase tracking-wider shadow-sm"
      ...
      <div className="flex items-center border border-slate-200 bg-slate-50 p-0.5 font-mono shadow-sm">
        ...
        {speedOptions.map((s) => (
          <button
            key={s}
            onClick={() => setSpeed(s)}
            className={`px-2 py-0.5 text-[10px] font-bold uppercase transition-all duration-150 border ${
              speed === s
